...
| # | 书 | 篇数 | 主要来源 |
|---|---|---|---|
| 1 | Agent 工程 | 35 | Agent 的工程实战+Orchestration+散篇；Knowledge 的 Agent 介绍/解码/提示词；DeepLearning/agent 的 n8n、nl2sql、openclaw |
| 2 | RAG 与 LangChain | 17 | Agent 的 LangChain 9 篇 + RAG 4 篇 + RAG 全景/RAGFlow/GraphRAG；向量数据库 |
| 3 | 深度学习 | 21 | DeepLearning 的 models/NLP/frame；Knowledge 的 JAX、字典学习、算子、vllm、模型对比、openbayes |
| 4 | 编程语言 | 29 | Grammar 的 python/PyQt/Matlab；Library 的 Python 库教程（pickle、pytest、PyYAML 等）；SQLAlchemy/Alembic |
| 5 | Web 开发 | 17 | Library 的 FastAPI/Flask/React/flutter；Knowledge 的 Jinja、Celery、streamlit、gradio、异步 API；Refine |
| 6 | 数据科学 | 13 | Library 的 transformers/torch/pandas/matplotlib；Python_Lib 的 numpy/scipy/sklearn；onnx |
| 7 | 构建与打包 | 10 | Platforms_Tools 的 PyInstaller/PyStand/pipx/uv/packageTools；setuptools、poetry、conda、打包 exe |
| 8 | 开发工具链 | 27 | Platforms_Tools 的 Docker/dev_tools/CLI；git&github、VScode、Blender、copier |
| 9 | 设计 | 14 | Design 全部（架构/类图/用例图/数据流图/原型图/作图）；Library/优秀图表学习；项目目录规范 |
| 10 | 工程实践 | 11 | Engineering 全部（可观测性/软件工程/DevOps/平台架构）；.env 安全、代码写作心得、模板规范 |
| 11 | 运维与服务器 | 27 | 服务器运维 11 篇、S3、Linux 4 篇、1panel/Coolify/rustdesk/Server Probe/clash、腾讯云/SSH/Nginx、PostgreSQL/redis/备份 |
| 12 | 项目实战 | 18 | crawler、hugo 建站 7 篇、单片机、Dify、软件自动更新、微信小程序 |
| 13 | 软件试用 | 30 | SoftTrial 17 + SoftUseExp 13（默认决策：独立成书） |
| 14 | 效率与文档 | 10 | markdown、word 技巧、windows/macos、overleaf、文档颜值、文档结构化、categories 概念 |
| 15 | 通识与生活 | 16 | 百科 6 篇、英语、上海地理、徒步、自学方法、尬聊、创业、菜谱、生活量化 |
| 16 | Vibe Coding | 7 | AI-Frontend 7 篇（含 ai-design-research 4 篇） |
| 17 | 面试八股 | 6 | 深度学习八股 5 篇 + LangChain 八股（默认决策：独立成书） |
| 18 | 阅读笔记 | 3 | 《大语言模型》读书笔记、瑞金拉曼血糖论文、近红外光谱知识点 |
| 19 | 科技月报 | 4 | 科技月报 2025/2026、每日资讯、技术热点追踪（专栏形态，按时间排，不排书序） |
| | **合计** | **315** | |
...
| 标题 | 旧位置 | 备注 |
|---|---|---|
| Matlab-基本语法 | `Grammar/Matlab/Matlab-基本语法` |  |
...
| Useful but not attempted | `Knowledge/others/Useful but not attempted` | ⚠️#13 |
...
13. `Knowledge/others/Useful but not attempted` — 技术问题清单，更像待写选题池，建议改造或合并
...
1. 过一遍本表：改书名、处理 ⚠️ 条目和合并候选（直接改 md 或口头说，我同步进脚本规则）
2. `zata.py create-category` 建 19 个新分类（含封面图），旧分类元数据保留到验证后再删
3. 迁移命令（参照 `tools/merge_categories.py` 的 dry-run/--apply 惯例）：git mv 目录 + 重写 categories，顺手修上面 5 处 front matter、处理 2 个空文件、删空目录
4. `hugo server` 全站点验，之后再做书视图前端（书页目录 + 篇尾续读）
